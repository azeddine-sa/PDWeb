...
if(isset($valider))
{
    ...
    if(empty($message))
    {
        ...
        $req=$bdd->prepare("SELECT id_user FROM users WHERE login=? limit 1");
        ...
        $req1=$bdd->prepare("SELECT id_user FROM users WHERE email=? limit 1");
        ...
        //sinon ajout de l'utilisateur dans la bd et redirection vers la page de connexion
            ...
            $requete = $bdd->prepare('INSERT INTO pdweb.users(email,login,pass,nom,prenom,token) VALUES(?,?,?,?,?,?)');

            $requete->execute(array($email,$login,$password,$nom,$prenom,$token));
