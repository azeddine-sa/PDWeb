        ...
        if(!$mail->send()){
          ...
          echo 'Erreurs:'.$mail->ErrorInfo;
           ...
           $message =  "Votre adresse email n'est pas encore confimée. Nous vous avons envoyé par courrier des instructions pour confirmer 
       		...
       		$requete1=$bdd->prepare('SELECT * FROM pdweb.recuppass WHERE email=:email');
       		...
       		if($nombre1==0){
       			...
       			$requete2=$bdd->prepare('INSERT INTO pdweb.recuppass(email,token) VALUES(:email, :token)');
       			...
       			$requete3= $bdd->prepare('UPDATE pdweb.recuppass SET token=:token WHERE email=:email');
